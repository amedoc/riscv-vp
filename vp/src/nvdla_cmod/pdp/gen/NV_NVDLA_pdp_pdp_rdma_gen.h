...
// File Name: NV_NVDLA_pdp_pdp_rdma_gen.h

#include "../../../nvdla_cmod/include/log.h"
#include "../../../nvdla_cmod/pdp/NV_NVDLA_pdp.h"
...
inline void
NV_NVDLA_pdp::csb2pdp_rdma_req_b_transport (int ID, NV_MSDEC_csb2xx_16m_secure_be_lvl_t* payload, sc_time& delay) {
    uint32_t addr;
    uint32_t data;
    uint8_t write;
    uint8_t nposted;
    ...
    if (true == is_there_ongoing_csb2pdp_rdma_response_) {
#pragma CTC SKIP
        FAIL(("NVDLA NV_NVDLA_pdp::csb2pdp_rdma_b_transport: there is onging CSB response."));
#pragma CTC ENDSKIP
    } else {
        is_there_ongoing_csb2pdp_rdma_response_ = true;
    }
...
#pragma CTC SKIP
        FAIL(("NVDLA NV_NVDLA_pdp::csb2pdp_rdma_b_transport: payload pointer shall not be NULL"));
#pragma CTC ENDSKIP
    }
    // Accessing registers
    register_access_result = PdpRdmaAccessRegister (addr, data, 0!=write);
    ...
    if ( (0==write) || (0!=nposted) ) {
        ...
        PdpRdmaSendCsbResponse(write, data, error_id);
    }
    is_there_ongoing_csb2pdp_rdma_response_ = false;
}

inline void
NV_NVDLA_pdp::PdpRdmaSendCsbResponse(uint8_t type, uint32_t data, uint8_t error_id) {
    ...
    NV_NVDLA_pdp_base::pdp_rdma2csb_resp_b_transport(&payload, b_transport_delay_);
}
