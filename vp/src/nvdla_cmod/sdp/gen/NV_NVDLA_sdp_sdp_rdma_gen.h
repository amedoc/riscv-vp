...
// File Name: NV_NVDLA_sdp_sdp_rdma_gen.h

#include "../../../nvdla_cmod/include/log.h"
#include "../../../nvdla_cmod/sdp/NV_NVDLA_sdp.h"
...
inline void
NV_NVDLA_sdp::csb2sdp_rdma_req_b_transport (int ID, NV_MSDEC_csb2xx_16m_secure_be_lvl_t* payload, sc_time& delay) {
    uint32_t addr;
    uint32_t data;
    uint8_t write;
    uint8_t nposted;
    ...
    if (true == is_there_ongoing_csb2sdp_rdma_response_) {
#pragma CTC SKIP
        FAIL(("NVDLA NV_NVDLA_sdp::csb2sdp_rdma_b_transport: there is onging CSB response."));
#pragma CTC ENDSKIP
    } else {
        is_there_ongoing_csb2sdp_rdma_response_ = true;
    }
...
#pragma CTC SKIP
        FAIL(("NVDLA NV_NVDLA_sdp::csb2sdp_rdma_b_transport: payload pointer shall not be NULL"));
#pragma CTC ENDSKIP
    }
    // Accessing registers
    register_access_result = SdpRdmaAccessRegister (addr, data, 0!=write);
    ...
    if ( (0==write) || (0!=nposted) ) {
        ...
        SdpRdmaSendCsbResponse(write, data, error_id);
    }
    is_there_ongoing_csb2sdp_rdma_response_ = false;
}

inline void
NV_NVDLA_sdp::SdpRdmaSendCsbResponse(uint8_t type, uint32_t data, uint8_t error_id) {
    ...
    NV_NVDLA_sdp_base::sdp_rdma2csb_resp_b_transport(&payload, b_transport_delay_);
}
