...
// File Name: NV_NVDLA_cdma_cdma_gen.h
...
NV_NVDLA_cdma::csb2cdma_req_b_transport (int ID, NV_MSDEC_csb2xx_16m_secure_be_lvl_t* payload, sc_time& delay) {
    ...
    if (true == is_there_ongoing_csb2cdma_response_) {
#pragma CTC SKIP
        FAIL(("NVDLA NV_NVDLA_cdma::csb2cdma_b_transport: there is onging CSB response."));
#pragma CTC ENDSKIP
    } else {
        is_there_ongoing_csb2cdma_response_ = true;
    }
...
#pragma CTC SKIP
        FAIL(("NVDLA NV_NVDLA_cdma::csb2cdma_b_transport: payload pointer shall not be NULL"));
#pragma CTC ENDSKIP
    }
    // Accessing registers
    register_access_result = CdmaAccessRegister (addr, data, 0!=write);
    ...
    if ( (0==write) || (0!=nposted) ) {
        ...
        CdmaSendCsbResponse(write, data, error_id);
    }
    is_there_ongoing_csb2cdma_response_ = false;
}

inline void
NV_NVDLA_cdma::CdmaSendCsbResponse(uint8_t type, uint32_t data, uint8_t error_id) {
    ...
    NV_NVDLA_cdma_base::cdma2csb_resp_b_transport(&payload, b_transport_delay_);
}
